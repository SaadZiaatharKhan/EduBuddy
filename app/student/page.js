...
import { Experience } from "@/components/3d/Experience"; // Adjust the path if necessary

function StudentPage({ user }) {
  const [openNav, setOpenNav] = useState(false);
  const [navValue, setNavValue] = useState("conversation");
  const [in3DSpace, setIn3DSpace] = useState(false);
        ...
        <div className="max-h-[20vh] relative">
          <div className="p-4 flex w-full justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <button
                className="bg-blue-500 hover:bg-blue-700 text-white rounded-md md:p-2 p-1"
                onClick={() => setIn3DSpace(true)}
              >
                Enter 3D Space
              </button>
              <button
      ...
      <div className="max-h-[80vh] w-full relative">
        {in3DSpace ? (
          <>
            ...
            <button
              className="absolute top-4 left-4 bg-white text-black p-2 rounded z-50"
              onClick={() => setIn3DSpace(false)}
            >
              Back
            </button>
